...
def trend_editing(request): # Для рендеринга страницы редактирования основных направлений деятельности
    page = Page.objects.get(url='Trend') # Получаем запись в таблице Page с именем Trend в поле url
    lis = TrendBasic.objects.all() # Получаем все записи с таблицы TrendBasic
...
def trend_edit(request, id): # Для рендеринга страницы изменения основного направления деятельности
    page = Page.objects.get(url='Trend') # Получаем запись в таблице Page с именем Trend в поле url
    lis = TrendBasic.objects.all() # Получаем все записи с таблицы TrendBasic
    trend = get_object_or_404(TrendBasic, id=id) # Получаем объект по переданному идентификатору
    if request.method == 'POST': # Если POST-запрос
        form = TrendBasicAddForm(request.POST, instance=trend) # Передаем объект для изменения
        if form.is_valid(): # Если форма валидна (все данные правильные)
            form.save() # Сохраняем изменения в базе данных
        ...
        form = TrendBasicAddForm(instance=trend) # Заполняем форму данными существующего объекта (записью с основным направлением)
        context = {'page': page, 'lis': lis, 'form': form} # Передаем шаблон
        return render(request, 'siteapp/Trend_edit.html', context) # И рендерим шаблон с передачей формы c изменением записи с основным направлением

def trend_delete(request, id): # Для рендеринга страницы подтверждения удаления одного из основных направлений деятельности
    page = Page.objects.get(url='Trend') # Получаем запись в таблице Page с именем Trend в поле url
    lis = TrendBasic.objects.all() # Получаем все записи с таблицы TrendBasic
    trend = get_object_or_404(TrendBasic, id=id) # Получаем объект по переданному идентификатору
    if request.method == 'GET': # Если простой GET-запрос
