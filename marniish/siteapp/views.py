...
class HistoryEditUpdateView(PageContextMixin, UpdateView): # Для рендеринга страницы изменения абзаца события
    page_url = 'About_edit' # Создаём наследованный из ContextMixin контекст из записи таблицы Page
    template_name = 'siteapp/About_edit.html' # Указываем расположение шаблона рендеринга
    form_class = HistoryEditingForm  # Указываем форму для редактирования абзаца события
...
class HistoryDeleteView(PageContextMixin, DeleteView): # Для рендеринга страницы подтверждения удаления абзаца события
    page_url = 'About_confirm_delete' # Создаём наследованный из ContextMixin контекст из записи таблицы Page
    template_name = 'siteapp/About_confirm_delete.html' # Указываем расположение шаблона рендеринга
    success_url = reverse_lazy('siteapp:About_editing') # После успешного сохранения возвращаемся на страницу редактирования
    queryset = History.objects.all() # Для получения всех записей таблицы History
    def get_context_data(self, **kwargs): # Для передачи данных в контекст
        context = super().get_context_data(**kwargs) # Получаем базовый контекст
