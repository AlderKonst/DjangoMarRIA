...
def trend_change(request): # Для рендеринга страницы основных направлений деятельности
    ...
    if request.method == 'GET': # Если простой GET-запрос
        ...
        context = {'page': page, 'lis': lis, 'form': form}  # Передаем шаблон
        if form.is_valid(): # Если форма валидна (все данные правильные)
            name = form.cleaned_data['name'] # Получаем значение поля name
            TrendBasic.objects.get_or_create(name=name) # Текст из формы с пунктом списка добавляем, если ещё нет
            return HttpResponseRedirect(reverse('siteapp:Trend_change')) # Перенаправляет на эту же страницу с уже внесёнными правками
